...
signUpForm.addEventListener('submit', (e) => {
    ...
    auth.createUserWithEmailAndPassword(email, password)
        ...
        .then(() => {
            userMessage.innerText = 'Account created successfully!';
        })
        .catch(err => {
...
claimBtn.addEventListener('click', () => {
    ...
    db.collection('users').doc(user.uid).get().then(doc => {
        const data = doc.data();
        let lastClaim = data.lastClaim ? new Date(data.lastClaim.toDate()) : null;
        const now = new Date();
...
watchAdBtn.addEventListener('click', () => {
    ...
    const points = Math.floor(Math.random() * 100) + 1; // 1 to 100 points
    db.collection('users').doc(user.uid).get().then(doc => {
        const data = doc.data();
        const coinsEarned = Math.floor(points/500)*2; // Every 1000 points = 2 coins
